...
!function(window, document, undefined) {
   ...
   */
    function testRunner() {
        ...
        for (var featureIdx in tests) if (tests.hasOwnProperty(featureIdx)) {
            ...
            if (featureNames = [], feature = tests[featureIdx], 
            ...
            nameIdx = 0; nameIdx < featureNames.length; nameIdx++) featureName = featureNames[nameIdx], 
   ...
   */
    function addTest(feature, test) {
        if ("object" == typeof feature) for (var key in feature) hasOwnProp(feature, key) && addTest(key, feature[key]); else {
            ...
            var featureNameSplit = feature.split("."), last = Modernizr[featureNameSplit[0]];
            if (// Again, we don't check for parent test existence. Get that right, though.
            2 == featureNameSplit.length && (last = last[featureNameSplit[1]]), 
            "undefined" != typeof last) // we're going to quit if you're trying to overwrite an existing test
            ...
            1 == featureNameSplit.length ? Modernizr[featureNameSplit[0]] = test : (// cast to a Boolean, if not one already
            /* jshint -W053 */
   ...
   */
    function injectElementWithStyles(rule, callback, nodes, testnames) {
        ...
        return style = createElement("style"), style.type = "text/css", 
        style.id = "s" + mod, (body.fake ? body : div).appendChild(style), 
        body.appendChild(div), style.styleSheet ? style.styleSheet.cssText = rule : style.appendChild(document.createTextNode(rule)), 
        div.id = mod, body.fake && (body.style.background = "", 
        body.style.overflow = "hidden", docOverflow = docElement.style.overflow, 
        docElement.style.overflow = "hidden", docElement.appendChild(body)), 
        ret = callback(div, rule), body.fake ? (body.parentNode.removeChild(body), 
   ...
   */
    function testDOMProps(props, obj, elem) {
        ...
        return elem === !1 ? props[i] : (item = obj[props[i]], 
   ...
   */
    ...
    function testProps(props, prefixed, value, skipValueTest) {
        ...
        if (skipValueTest = is(skipValueTest, "undefined") ? !1 : skipValueTest, 
        !is(value, "undefined")) {
        ...
        before = mStyle.style[prop], contains(prop, "-") && (prop = cssToDOM(prop)), 
        mStyle.style[prop] !== undefined) {
            // If value to test has been passed in, do a set-and-check test.
            // 0 (integer) is a valid property value, so check that `value` isn't
            // undefined, rather than just checking it's truthy.
            if (skipValueTest || is(value, "undefined")) return cleanElems(), 
            "pfx" == prefixed ? prop : !0;
            ...
            // CSS.supports()
            if (mStyle.style[prop] != before) return cleanElems(), 
            "pfx" == prefixed ? prop : !0;
        }
        return cleanElems(), !1;
    }
...
*/
    Modernizr.addTest("csscalc", function() {
        var prop = "width:", value = "calc(10px);", el = createElement("a");
        return el.style.cssText = prop + prefixes.join(value + prop), 
        !!el.style.length;
    });
    /**
   ...
   */
    ModernizrProto.on = function(feature, cb) {
        ...
        this._l[feature] || (this._l[feature] = []), // Push this test on to the listener list
        this._l[feature].push(cb), // If it's already been resolved, trigger it on next tick
        Modernizr.hasOwnProperty(feature) && // Next Tick
        setTimeout(function() {
   ...
   */
    ModernizrProto._trigger = function(feature, res) {
        if (this._l[feature]) {
            ...
            setTimeout(function() {
                var i, cb;
                for (i = 0; i < cbs.length; i++) (cb = cbs[i])(res);
...
!*/
    testStyles("#modernizr { width: 50vw; }", function(elem) {
        var width = parseInt(window.innerWidth / 2, 10), compStyle = parseInt((window.getComputedStyle ? getComputedStyle(elem, null) : elem.currentStyle).width, 10);
...
*/
    ...
    setClasses(classes), delete ModernizrProto.addTest, 
